    ...
    return (
        ...
        <Grid container minHeight='100vh' justifyContent='space-between' columnSpacing={{ xs: 0, sm: 0, md: 0 }}>
            <Grid item md={1} xs={0} order={{ xs: 1 }} sx={{ display: { xs: 'none', md:'block' } }}>
                ...
                <Typography sx={{mt: {xs: 12, md: 42}, textTransform: 'uppercase', transform: 'rotate(180deg)', writingMode: 'vertical-lr'}}>{artSeriesSelected.title}</Typography>
                </Container>
            </Grid>

            <Grid item md={7} xs={12} order={{ xs: 3 }}>
                <Box sx={{ width: {xs: '90%', md: '95%'}, mx: 'auto', bgcolor: 'background.paper', pb: 6}}>
                    <Typography mt={{xs: 12, md: 40}} fontSize={{xs: 30, md: 50}} component='h1' variant='h2' color='text.primary'>{artSeriesSelected.title}</Typography>
                    <FormControl sx={{ minWidth: 200, mt: {xs: 12, md: 25}, mb: 7 }}>
                        ...
                        <Select
                            ...
                            defaultValue={'Города'}
                        >
                            <MenuItem value={0}>Все работы</MenuItem>
                            <MenuItem value={1}>Города</MenuItem>
                            <MenuItem value={2}>Графика</MenuItem>
                            <MenuItem value={3}>Женская эстетика</MenuItem>
                            <MenuItem value={4}>Цветы</MenuItem>
                            <MenuItem value={5}>Ихтис</MenuItem>
                            <MenuItem value={6}>Фрукты</MenuItem>
                            <MenuItem value={7}>Абстракция</MenuItem>
                            <MenuItem value={8}>Тайны океана</MenuItem>
                            <MenuItem value={9}>Натюрморт</MenuItem>
                            <MenuItem value={10}>Пейзажи</MenuItem>
                            <MenuItem value={11}>Филосовская фантазия</MenuItem>
                        </Select>
                    </FormControl>
                    <Outlet />
                </Box>
