...
const Collection = ({ t }) => {
    ...
    const [artSeriesSelected, setArtSeriesSelected] = React.useState(null);
    let navigate = useNavigate();

    React.useEffect(() => {
        setArtSeriesSelected(seriesSlug == null ? artSeries.find(series => series.seriesPath === 'all') : artSeries.find(series => series.seriesPath === seriesSlug));
    }, [seriesSlug, artSeries]);

    const handleChange = (event) => {
        let artSeriesSelection = artSeries.find(series => series.seriesId === event.target.value);
                        ...
                        <Box sx={{ width: {xs: '90%', md: '95%'}, mx: 'auto', bgcolor: 'background.paper', pb: 6}}>
                            ...
                            <FormControl sx={{ minWidth: 200, mt: {xs: 12, md: 25}, mb: 7 }}>
                                ...
                                <Select
                                    ...
                                    value={artSeriesSelected.seriesId}
